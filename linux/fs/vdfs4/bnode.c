	...
	struct vdfs4_btree *btree, struct vdfs4_bnode *bnode, __le32 *checksum)
{
	...
	*checksum = crc_on_bnode;
	...
	case VDFS4_BTREE_XATTRS:
		...
		break;
	default:
		VDFS4_ERR("(%s) Invalid btree_type : %d\n", get_sid_from_sbi(sbi), btree->btree_type);
		VDFS4_BUG(sbi);
	}
	...
	if (crc_on_bnode != crc_on_hashtable[index]) {
		VDFS4_ERR("(%s) wrong bnode hash bnode:%x hashtable:%x\n",
			  get_sid_from_sbi(sbi), crc_on_bnode, crc_on_hashtable[index]);
		return 0;
	}
	return 1;
}
		...
		const char *fmt, ...)
{
	...
	panic("(%s): forced kernel panic after fatal error\n", device);
#else
...
do_reread:
	...
	if (!data) {
		VDFS4_WARNING("unable to vmap %d pages", btree->pages_per_node);
		ret = -ENOMEM;
	...
	if (!create && bnode->node_id != 0 &&
		...
		VDFS4_ERR("Wrong bnode magic(%s,type:%u,mode:%u):magic:%#x,addr:0x%p,inode=%lu,bnode=%d",
			  get_sid_from_sbi(btree->sbi), btree->btree_type,
	...
	if (!create && !PageDirty(bnode->pages[0]) &&
		...
		real_version = VDFS4_BTREE_VERSION(data);
		version = vdfs4_get_page_version(bnode->host->sbi,
						 btree->inode, page_idx);
		if (real_version != version) {
			VDFS4_ERR("METADATA PAGE VERSION MISMATCH(%s, type:%u,mode:%u):inode_num-%lu,bnode_num-%u,addr:0x%p, must be - %u.%u, real - %u.%u",
				  get_sid_from_sbi(btree->sbi), btree->btree_type,
				  bnode->mode, btree->inode->i_ino,
				  bnode->node_id, data,
	...
	if ((!create) && (!PageChecked(bnode->pages[0]) &&
		...
		if (!verify_hash || from_disk != calculated) {
			VDFS4_ERR("CRC mismatch(%s,type:%u,mode:%u,verify_hash:%d):(Btree Inode id:%lu,Bnode id:%u,addr:0x%p)(calc:%#x,read:%#x)",
				  get_sid_from_sbi(btree->sbi), btree->btree_type,
	...
	if (reread_count != VDFS4_META_REREAD_BNODE)
		VDFS4_ERR("(%s) wrong data read from mmc, need to investigate (try %d/%d)",
		...
		__u32 node_id, enum vdfs4_get_bnode_mode mode, int wait)
{
	...
	if (!bnode) {
		...
		if (mode == VDFS4_BNODE_MODE_RW && bnode->ref_count > 2) {
			VDFS4_ERR("(%s) bnode %d ref_count %d",
				  get_sid_from_sbi(btree->sbi),
				  bnode->node_id,
				  bnode->ref_count);
		}
...
int vdfs4_check_bnode_reserve(struct vdfs4_btree *btree, int force_insert)
{
	...
	used_bnodes = (pgoff_t)btree->bitmap->bits_num
			- (pgoff_t)btree->bitmap->free_num;
		...
		struct vdfs4_btree *btree, __u64 version)
{
	...
	bnode_data = vdfs4_vm_map_ram(page, btree->pages_per_node,
				-1, PAGE_KERNEL);
	if (!bnode_data) {
		VDFS4_ERR("(%s) can not allocate virtual memory",
			  get_sid_from_sbi(btree->sbi));
		return -ENOMEM;
	}
#ifdef CONFIG_VDFS4_META_SANITY_CHECK
