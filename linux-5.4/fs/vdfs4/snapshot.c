...
static bool is_lackage_meta_free_blocks_to_use(struct vdfs4_sb_info *sbi,
					       int threshold);

static inline bool is_commit_required(struct vdfs4_sb_info *sbi)
{
	if (!sbi->free_blocks_count &&
	    sbi->fsm_info && sbi->fsm_info->next_free_blocks) {
		VDFS4_NOTICE("(%s) commit required by fsm ", sbi->sb->s_id);
		return true;
	}

	if (is_lackage_meta_free_blocks_to_use(sbi, 5)) {
		VDFS4_NOTICE("(%s) commit required by meta free block lackage",
			     sbi->sb->s_id);
		return true;
	}
...
static int get_table_version(struct vdfs4_sb_info *sbi,
	...
	if (!page) {
		VDFS4_WARNING("(NOMEM)(%s) cannot alloc page for table version\n",
...
read_basetable:
	...
	table = vdfs4_vmalloc_for_dma((unsigned long int)(DIV_ROUND_UP(
				tables_size_in_bytes,
	...
	ret = vdfs4_table_IO(sbi, table, tables_size_in_bytes,
			REQ_OP_READ | REQ_META | REQ_PRIO, &table_start);
	if (ret) {
		VDFS4_ERR("(%s) Failed to table IO.", get_sid_from_sbi(sbi));
		vfree(table);
		return ret;
	}
	...
	if (ret) {
		VDFS4_ERR("(%s) Failed to restore exsb\n", get_sid_from_sbi(sbi));
		vfree(table);
		return ret;
	}
...
static int load_extended_table(struct vdfs4_sb_info *sbi, sector_t start)
{
	...
	if (!snapshot->extended_table) {
		snapshot->extended_table = vdfs4_vmalloc_for_dma(PAGE_SIZE);
		if (!snapshot->extended_table)
			return -ENOMEM;
	}

	return vdfs4_table_IO(sbi, snapshot->extended_table, PAGE_SIZE,
			REQ_OP_READ | REQ_META | REQ_PRIO, &start);
}
...
retry_basetable_write:
		ret = vdfs4_table_IO(sbi, (char *)table + PAGE_SIZE,
			...
			REQ_OP_WRITE | REQ_PREFLUSH | REQ_FUA | REQ_META | REQ_PRIO,
			&s_iblock);
		if (ret) {
			if (--retry_count >= 0) {
...
retry_transtable_write:
	ret = vdfs4_table_IO(sbi, table, PAGE_SIZE,
		REQ_OP_WRITE | REQ_PREFLUSH | REQ_FUA | REQ_META | REQ_PRIO, &iblock);
	if (ret) {
		if (--retry_count >= 0) {
...
static __u32 get_minimal_meta_expand_step(struct vdfs4_sb_info *sbi)
{
	...
	__u32 minimal_expand_step = 2 * (3 * (1u <<
		...
		__u64 new_table_index)
{
	...
	size_t bitmaps_length, old_bitmap_length;
	...
	if (((tables_size & (PAGE_SIZE - 1)) +
		...
		void *new_table = vdfs4_vmalloc_for_dma(
					tables_in_pages << PAGE_SHIFT);
		...
		if (ret) {
			...
			if (!force_insert &&
				...
				((free_blocks - (long)new_blocks_count) <
						10L * get_minimal_meta_expand_step(sbi)))
				return ret;
...
static long meta_area_free_blocks(struct vdfs4_sb_info *sbi)
{
	...
	u32 free_blocks = le32_to_cpu(vdfs_sb->exsb.meta_tbc);
	ino_t ino_n;

	for (ino_n = VDFS4_FSFILE; ino_n <= VDFS4_LSFILE; ino_n++) {
		u32 node_count, blocks_per_node;

		if (is_tree(ino_n))
			blocks_per_node = 1 << (sbi->log_blocks_in_page
						+ sbi->log_blocks_in_leb);
		else
			blocks_per_node = 1 << sbi->log_blocks_in_page;

		node_count = (u32)VDFS4_LAST_TABLE_INDEX(sbi, ino_n) + 1;
		free_blocks -= blocks_per_node * node_count;
	}

	return (long)free_blocks;
}
...
{
	struct vdfs4_snapshot_info *snapshot = sbi->snapshot_info;
	unsigned long free_blocks, cow_blocks, free_to_use;
	unsigned long threshold_in_blocks;

	down_read(&snapshot->tables_lock);
	free_blocks = meta_area_free_blocks(sbi);
	cow_blocks = snapshot->cow_blocks;
	up_read(&snapshot->tables_lock);

	/* Checking abnormal case. It cannot determin shortage or not. */
	if (unlikely(free_blocks < cow_blocks)) {
		VDFS4_WARNING("(%s) Invalid cow blocks counting(%lu,%lu)",
			      sbi->sb->s_id, free_blocks, cow_blocks);
		return false;
	}

	free_to_use =  free_blocks - cow_blocks;
	threshold_in_blocks = free_blocks / 100;
	threshold_in_blocks *= threshold;

	return (free_to_use < threshold_in_blocks) ? true : false;
}
...
int vdfs4_is_enospc(struct vdfs4_sb_info *sbi, int threshold)
{
	...
	down_read(&snapshot->tables_lock);
	ttb = (long)le32_to_cpu(vdfs_sb->exsb.meta_tbc);
