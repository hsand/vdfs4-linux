...
/* decompression function type */
typedef int (*decomp_fn_t)(void *src, void *dst, size_t offset,
...
enum vdfs4_read_type {
	...
	VDFS4_FBASED_READ_PROFILED, /* file based profoiled chunk read */
#endif
	VDFS4_READ_NR,
};
...
#define VDFS4_INLINE_DATA_FILE  19
#define VDFS4_COMP_INLINE_DATA_FILE	20
...
#if defined(CONFIG_ARM64) && defined(CONFIG_NO_SWIOTLB_BUFFER)
#define vdfs4_vmalloc_for_dma(...) ({				\
	unsigned noio_flags = memalloc_noio_save();		\
	void *ret = vmalloc_32(__VA_ARGS__);			\
	int retry = VDFS4_VMEM_RETRY_CNT;			\
	while (unlikely(!ret) && retry--) {			\
		VDFS4_WARNING("(NOMEM)(%s():%d, %d):vmalloc_32\n",\
			      __func__, __LINE__, retry);	\
		msleep(VDFS4_VMEM_RETRY_INTERVAL);		\
		ret = vmalloc_32(__VA_ARGS__);			\
	}							\
	memalloc_noio_restore(noio_flags);			\
	ret;							\
})
#else
#define vdfs4_vmalloc_for_dma(...) vdfs4_vmalloc(__VA_ARGS__)
#endif
 ...
 * discard command.
 */
#define VDFS4_DELAYED_DISCARD_MAGIC (0xDEEDDCAD)
...
/*
 * @brief	compressed inline data information
 */
struct vdfs4_comp_inline_data_info {
	decomp_fn_t decomp_fn;
	unsigned int comp_size;
	char data[VDFS4_COMP_INLINE_DATA_MAX_SIZE];
};
...
struct vdfs4_file_based_info {
	...
	u8 force_path;
	/* compression type */
	enum compr_type compr_type:8;
	/* hash type */
	enum hash_type hash_type:8;
	/* signature type */
	enum sign_type sign_type:8;
	/* hash length */
	size_t hash_len;
...
struct vdfs4_inode_info {
	...
	union {
		...
		/** The inode inline data */
		char inline_data[VDFS4_INLINE_DATA_MAX_SIZE];
		/** The inode compressed inline data */
		struct vdfs4_comp_inline_data_info comp_inline_data_info;
	};
...
struct vdfs4_snapshot_info {
	...
	/* block count only for cow */
	unsigned long cow_blocks;
...
static inline char *get_sid_from_sbi(struct vdfs4_sb_info *sbi)
{
	return sbi->sb->s_id;
}

static inline char *get_sid_from_inode(struct inode *inode)
{
...
{
	return file_inode(iocb->ki_filp);
}
...
{
	return (inode->i_sb->s_time_gran < NSEC_PER_SEC) ? current_time(inode)
...
static inline int vdfs4_is_inline_data_inode(struct inode *inode)
{
	return is_vdfs4_inode_flag_set(inode, VDFS4_INLINE_DATA_FILE);
}

static inline int vdfs4_is_comp_inline_data_inode(struct inode *inode)
{
	return is_vdfs4_inode_flag_set(inode, VDFS4_COMP_INLINE_DATA_FILE);
}

static inline int vdfs4_is_fast_symlink_inode(struct inode *inode)
{
	return inode->i_link ? 1 : 0;
}

static inline __u64 vdfs4_generate_version(struct vdfs4_sb_info *sbi)
{
	struct vdfs4_layout_sb *vdfs4_sb = sbi->raw_superblock;
	__u64 version;

	version = (__u64)le32_to_cpu(vdfs4_sb->exsb.mount_counter) << 32;
	version |= sbi->snapshot_info->sync_count;
...
bool vdfs4_bnode_sanity_check(struct vdfs4_bnode *bnode);
bool vdfs4_meta_sanity_check_bnode(void *bnode_data, struct vdfs4_btree *btree,
		pgoff_t index);
#endif
/**
...
decomp_fn_t vdfs4_get_decomp_fn(enum compr_type);
...
#define VDFS4_BITMAP_VERSION(data) ((((__le64) \
	le32_to_cpu(((struct vdfs4_meta_block *)(data))->mount_count)) << 32) \
	| (le32_to_cpu(((struct vdfs4_meta_block *)(data))->sync_count)))

#define VDFS4_BTREE_VERSION(node) \
	(*((__le64 *)(((struct vdfs4_gen_node_descr *)(node))->version)))
...
static inline void vdfs4_assert_inode_lock(struct inode *inode)
{
	lockdep_assert_held_write(&inode->i_rwsem);
}

static inline void vdfs4_assert_inode_lock_shared(struct inode *inode)
{
	lockdep_assert_held_read(&inode->i_rwsem);
}
