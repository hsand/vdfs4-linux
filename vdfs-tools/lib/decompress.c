...
int decompress_zstd(unsigned char *ibuff, int ilen,
		    unsigned char *obuff, int *olen)
{
	size_t sz;

	sz = ZSTD_decompress(obuff, *olen, ibuff, ilen);
	if (ZSTD_isError(sz)) {
		log_error("zstd decompress error(o:%d,i:%d,err:%s)\n", *olen,
			  ilen, ZSTD_getErrorName(ZSTD_getErrorCode(sz)));
		return -1;
	}
		...
		int *compress_type, off_t file_size_offset, int *log_chunk_size)
{
	...
	*log_chunk_size = le32_to_cpu(descr->log_chunk_size);
	...
	else if (!memcmp(descr->magic + 1, VDFS4_COMPR_ZSTD_FILE_DESCR_MAGIC,
			sizeof(descr->magic) - 1))
		*compress_type = VDFS4_COMPR_ZSTD;
...
int decompress_data(enum compr_type type, unsigned char *ibuff, int ilen,
		    unsigned char *obuff, int *olen)
{
	return decompressor[type](ibuff, ilen, obuff, olen);
}
...
int (*decompressor[VDFS4_COMPR_NR])(unsigned char *ibuff, int ilen,
	...
	[VDFS4_COMPR_ZSTD]	= decompress_zstd,
};
