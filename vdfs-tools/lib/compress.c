...
size_t compr_align[VDFS4_COMPR_NR][ALIGN_NR] = {
		...
		[VDFS4_COMPR_ZSTD]	= {[ALIGN_START] = 0,	[ALIGN_LENGTH] = 0},
...
#define VDFS4_ZSTD_COMPR_LEVEL (10)
...
int add_compression_info(int dst_fd, int chunks_num, off_t file_size,
		...
		vdfs4_hash_algorithm_func *hash_alg, int hash_len)
{
	...
	*c_size += aligned_size;
	...
	case VDFS4_COMPR_ZSTD:
		magic = VDFS4_COMPR_ZSTD_FILE_DESCR_MAGIC;
		break;
...
int compress_data(enum compr_type type, unsigned char *in, int ilen,
		  unsigned char *out, int olen, int *compressed_size)
{
	return compressor[type](in, ilen, out, olen, compressed_size);
}
...
static int prepare_file_descr(struct vdfs4_comp_file_descr *descr,
		int compress_type, int chunks, off_t src_file_size,
...
/* the compress_file returns chunks count */
static int compress_file(struct vdfs4_sb_info *sbi, char *src_path, int src_fd,
...
compress_retry:
	...
	*ext = ext_table;
	...
	no_written_chunks = 0;
	off_t dst_offset = (off_t)get_metadata_size(sbi);
	lseek(dst_fd, dst_offset, SEEK_SET);
	unpacked_offset = 0;

	/* write uncompressed ordered chunks */
	if (pfile) {
		int i;
		log_info("profiled file(%-100s) : chunk cnt(%d)", pfile->path, pfile->chunk_count);
		for (i = 0; i < pfile->chunk_count; i++) {
			int ext_idx = pfile->chunk_order[i];
			size_t aligned_len = ext_table[ext_idx].len_bytes;
			...
			if (align)
				aligned_len = ALIGN(ext_table[ext_idx].len_bytes, align);
			lseek(tmp_uncompr_fd, ext_table[ext_idx].start, SEEK_SET);
			...
			ext_table[ext_idx].start = unpacked_offset;
			unpacked_offset += aligned_len;
			write(dst_fd, in, avail_in);

			no_written_chunks++;
			ext_table[ext_idx].profiled_prio = i;

			//If it is not last of file and it is last chunk and not
			//aligned with block size, add padding data for block size align
			if (no_written_chunks < chunks &&
			    ext_idx == count - 1 && (aligned_len % sbi->block_size)) {
				int padding_sz = sbi->block_size - aligned_len;
				ret = write_zero_area(dst_fd, padding_sz);
				if (ret) {
					ret = -1;
					goto err_write_exit;
				}
				*new_file_size += padding_sz;
				unpacked_offset += padding_sz;
			}

		}
	}

	/* write uncompressed unordered chunks */
	for (ret = 0; ret < count; ret++) {
		if (pfile && memcmp(ext_table[ret].magic, VDFS4_COMPR_EXT_MAGIC,
				    sizeof(ext_table[ret].magic)))
			continue;
		if ((ext_table[ret].flags & VDFS4_CHUNK_FLAG_UNCOMPR)) {
	...
	*new_file_size += aligned_size;
	...
	if (IS_FLAG_SET(sbi->service_flags, ENABLE_DEDUPLICATION)) {
		__u64 begin = find_data_duplicate(sbi, sbi->disk_op_image.file_id,
				sbi->disk_op_image.file_id, src_path,
	...
	add_data_range(sbi, src_path, (*block << PAGE_SHIFT), *new_file_size);
		...
		int do_encrypt, const struct profiled_file* pfile)
{
	...
	*rsl_filesize = 0;
	...
	if (need_compress) {
		...
		if (thread_num >= 0)
			chunks_num = compress_file(sbi, src_filename, src_fd,
					tmp_uncompr_fd,
...
int compress_zstd_chunk(unsigned char *ibuff, int ilen,
		unsigned char *obuff, int olen, int *comp_size)
{
	size_t ret;

	ret = ZSTD_compress(obuff, olen, ibuff, ilen, VDFS4_ZSTD_COMPR_LEVEL);
	if (ZSTD_isError(ret)) {
		log_error("zstd decompress error(o:%d,i:%d,err:%s)\n", olen,
			  ilen, ZSTD_getErrorName(ZSTD_getErrorCode(ret)));
		return ret;
	}
...
int (*compressor[VDFS4_COMPR_NR])(unsigned char *ibuff, int ilen,
	...
	[VDFS4_COMPR_ZSTD]	= compress_zstd_chunk,
};
...
int parse_list_cmd(struct install_task *sqt, char *cmd_str)
{
	...
	while (cmd) {
		...
		if (next_cmd)
			memcpy(next_cmd, "\0", 1);
		if (!strncmp("COMPRESS", cmd, strlen("COMPRESS") + 1)) {
		...
		} else if(!strncmp("NOCOMPRESS", cmd, strlen("NOCOMPRESS") + 1)) {
...
void compress_file_thread(void *arg)
{
	...
	while (!tinfo->exit) {
		...
		if ((tinfo->tune_target == TUNE_PROFILED_FILES && !pfile) ||
		    (tinfo->tune_target == TUNE_NORMAL_FILES && pfile)) {
			pthread_mutex_unlock(&tinfo->compr_file_mutex);
			ret = 0;
			goto exit;
		}
...
static int compress_dir(struct vdfs4_sb_info *sbi,
		...
		enum tune_file_type tune_target)
{
	...
	while (!ret && data) {
		...
		if (S_ISDIR(info.st_mode)) {
			...
			ret = compress_dir(sbi, &obj_ptr, tune_target);
			if (ret)
				goto exit;
			...
			thread_file[tnum].tune_target = tune_target;
			pthread_mutex_lock(&files_count_mutex);
			count++;
	...
	for (pos = install_task_list->next; pos != install_task_list;
		...
		if ((ptr->cmd & CMD_COMPRESS)) {
			...
			if (S_ISREG(stat_info.st_mode)) {
				...
				ret = compress_dir(sbi, ptr, tune_target);
				if (ret)
					goto exit;
			} else {
